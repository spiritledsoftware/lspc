...
def payload_files(skill_dir: Path) -> list[tuple[str, Path]]:
    ...
    for path in skill_dir.rglob("*"):
        ...
        if relative == ".lspc-managed.json" or not relative or ".." in PurePosixPath(relative).parts:
...
def skill_digest(files: list[tuple[str, Path]]) -> str:
    digest = hashlib.sha256(b"lspc-skill-v1\0" + len(files).to_bytes(8, "big"))
...
def write_archive(path: Path, entries: list[tuple[str, Path]], manifest: bytes, prefix: str, kind: str) -> None:
    ...
    contents = [(relative, source.read_bytes(), 0o755 if relative.startswith("lspc") else 0o644) for relative, source in entries]
...
def verify_archive(path: Path, manifest: bytes, prefix: str, kind: str) -> None:
    ...
    if not expected <= names or not any(name.startswith(f"{prefix}/skills/lspc/") for name in names):
