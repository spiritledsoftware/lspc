...
This clone currently has no Git remote. Configure a GitHub remote before relying on `gh` to infer the repository.
...
Infer the repo from `git remote -v`; `gh` does this automatically when run inside a clone with a configured remote.

## Pull requests as a triage surface

**PRs as a request surface: no.** Set this to `yes` if the repo treats external PRs as feature requests.
...
- **Child ticket**: a GitHub sub-issue linked to the map. If sub-issues are unavailable, add it to a task list and put `Part of #<map>` at the top. Apply a `wayfinder:<type>` label.
- **Blocking**: use GitHub issue dependencies. If unavailable, add `Blocked by: #<n>, #<n>` at the top.
- **Frontier query**: find the first open, unblocked, and unassigned child in map order.
